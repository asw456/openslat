...
public class LossR {
	...
	private double integrand(double t, double loss) {
		...
		if (calculationOptions.isCollapse()) {
			double lnMeanLC = calculationOptions.getStructure().getLossCollapse()
					.meanLoss();
			double sigmaLC = calculationOptions.getStructure().getLossCollapse()
					.sigmaLoss();
			double probLIMC = new LogNormalDistribution(lnMeanLC, sigmaLC)
					.cumulativeProbability(loss);

			double probCollapse = calculationOptions.getStructure().getPc().getPcim()
					.probability(im);
